...
<div class="card card-body rounded-0" style="max-width: 30rem;">
  <form method="POST" action="<?= base_url('dashboard/admin/masterdata/penerbit/'); ?>">
    ...
    <div class="mb-3">
      ...
      <input type="text" class="form-control <?= $validation->hasError('penerbit') ? 'is-invalid' : '' ?>" id="penerbit" name="penerbit" value="<?= $penerbit["penerbit"] ?>">
    </div>
    <div class="mb-3">
      <label for="kode_penerbit" class="form-label">Kode Penerbit</label>
      <input type="text" class="form-control <?= $validation->hasError('kode_penerbit') ? 'is-invalid' : '' ?>" id="kode_penerbit" name="kode_penerbit" value="<?= $penerbit["kode_penerbit"] ?>">
    </div>
