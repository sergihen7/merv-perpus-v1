  ...
  <form method="POST" action="<?= base_url("dashboard/admin/notif/save") ?>">
    ...
    <div class="mb-3">
      ...
      <select name="level_akses" id="level_akses" class="form-select <?= ($validation->hasError('level_akses')) ? 'is-invalid' : '' ?>">
        ...
        <option <?= (old("level_akses") == '0') ? 'selected' : '' ?> value="0">Semua</option>
        <option <?= (old("level_akses") == '1') ? 'selected' : '' ?> value="1">Admin</option>
        <option <?= (old("level_akses") == '2') ? 'selected' : '' ?> value="1">Member</option>
      </select>
    </div>
